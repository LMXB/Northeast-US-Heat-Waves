...
%NOTE::: Can enter nLines manually if \n character not recognized in file
%-- see line 235
%
...
if nargin > 1
  ...
  for ii =  1 : numel( params )
    ...
    %If we have multiple matches make sure we don't have a single unambiguous match before throwing an error
    if numel( result ) > 1
...
%Calculate number of lines or enter manually
pos             = ftell( fid );
if pos == -1
  msg = ferror( fid );
  fclose( fid );
