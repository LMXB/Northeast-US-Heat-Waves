function [cgridpts,closelandgridpts] = wnarrgridpts(deslat,deslon,oceanok,returncloselandgridpts)
...
%Central Park a.ka. the center of the NYC metro area: 40.78 N, 73.97 W

cgridpts=zeros(4,3); %columns are lat & lon of point, and then its fractional weight
closelandgridpts=zeros(10,6);
xnumber=3077; %number of gridpts we want to display as 'pretty close' (default: 10)
    ...
    while i-nopf<=4 && endthis==0
        if endthis==0
            ...
            if lsmask(closestxnumber(i+nopf,3),closestxnumber(i+nopf,4))==0 
                ...
                if i+nopf==xnumber %can't find 4 land pts among xnumber closest
        ...
        if cgrc==4 %happily found 4 pts
            endthis=1;
        elseif i+nopf==xnumber %couldn't find 4 land pts
            endthis=1;disp(sprintf('Could not find four land pts. Using just %d land pts among the top %d\n',cgrc,xnumber));
        end
        %disp('End this');disp(endthis);disp('i');disp(i);disp('i+nopf');disp(i+nopf);
    end
    %disp(closestxnumber);
else
    cgridpts(:,1)=closestxnumber(1:4,3);
    cgridpts(:,2)=closestxnumber(1:4,4);
    for i=1:4
        cgridpts(i,3)=sum(closestxnumber(1:4,5))/closestxnumber(i,5);
    end
end
