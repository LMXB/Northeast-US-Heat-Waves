...
%Transpose compares stations across dates (7x7); untransposed compares dates
...
ax=axis;
%axis(axis);
t=text(Xt,ax(2)*ones(1,length(Xt))+0.2,cities(1:numcities,:));
set(t,'HorizontalAlignment','right','VerticalAlignment','bottom', ...
    'Rotation',45,'FontSize',11);
t=text(ones(1,length(Xt))*(ax(1)-1),Xt+0.5,cities(1:numcities,:));
set(t,'HorizontalAlignment','left','VerticalAlignment','top', ...
    'Rotation',45,'FontSize',11);
title(sprintf('Stn MaxT Correl Matrix for %d Hot Days',size(stnobshotdays,1)),'FontSize',16);
