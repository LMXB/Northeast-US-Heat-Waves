...
%and makes plots of various kinds
...
%negative phases
%Although I'd rather not, I really need to consider each month on its own,
%not the whole summer together, as readings can change considerably
%month-to-month
...
for selstn=stniwf:stniwl %defined for marquee group only (would need to change above loops for others)
    ...
    for cat=catiwf:catiwl
        v=variabs{cat};if cat==2;sup=1;else sup=0;end
    ...
    fgtitle=sprintf('Deviations in warm %s %s under %s+',monlist{selmon-5},variabs2{cat},indiceslist{selind});
    ...
    set(fg,'Color',[1 1 1]);axis off;title(fgtitle,'FontSize',16,'FontWeight','bold');
    ...
    markerlist={'v';'o';'s'};
    jqqqxxx=eval(['j' char(variabs{cat}) '901']);
    jjaqqqx=eval(['jja' char(variabs{cat}) '1']);
    jjaqqq=eval(['jja' char(variabs{cat})]);
