...
%Upon starting Matlab:
    %load workspace_analyzenycdata, the five JJA avgs, organiznarrheatwaves arrays, and the cluster-specific arrays
...
%load('/Users/colin/Documents/General_Academics/Research/Exploratory_Plots/Saved_Variables_etc/narravgjjagh500');narravgjjagh500=narravgjjagh;
%load('/Users/colin/Documents/General_Academics/Research/Exploratory_Plots/Saved_Variables_etc/narravgjjauwnd1000');narravgjjauwnd1000=narravgjjauwnd;
%load('/Users/colin/Documents/General_Academics/Research/Exploratory_Plots/Saved_Variables_etc/narravgjjavwnd1000');narravgjjavwnd1000=narravgjjavwnd;
...
%load('/Users/colin/Documents/General_Academics/Research/Exploratory_Plots/Saved_Variables_etc/organizenarrheatwavessaufseasonality');
...
varsdes=[1;2;3;4;5];                        %variables to compute and prepare for plotting
    ...
    viwf=4;viwl=5;                          %the range of variables to analyze on this run
...
presleveliw=[1;1;3;2;2];            %pressure levels of interest for variables, at least on this run, with indices referring to preslevels
...
savetotalsintoarrays=0;             %whether to save 'totals' computed in the prior two sections into the arrays needed in the following section (5 sec)
                                        ...
                                        %1-8+10, 101-103, 201-203, 301-304, 401-424 (3 seasons & 8 possible hourstodo), or 501-502
    if allhwdays==1
        clmin=10;clmax=10;
    elseif tempevol==1
...
plotheatwavecomposites=0;           %whether to plot NARR heat-wave data (2 min)
    ...
    clustdes=505;                     %cluster to plot -- 0 (full unclustered data), anything from 1 to k (bottom-up clusters), 
                                      %10 (hours, for all heat-wave days),
                                      %101-103 (days, for temporal evolution), 201-203 (seasons, for seasonality),
                                      %301-304 (hours, for diurnality), 401-424 (combined seasonality-diurnality), 501-516 (moistness)
    ...
    mapregion='usa-exp';            %size of map region for plots made on this run (see Section I for sizes)
...
calcarealextentforscatterplot=1;    %whether to compute & plot scatterplot of spatial extent of 500-hPa anomalies of a variable vs magnitude of NYC T for NYC heat waves
...
for i=501:516 %501-508 is moisture category 1 hours 1-8
    ...
    for categ=1:1
        ...
        while rowtomake<=numtodo && rowtosearch<=vecsize
            ...
            if compositehwdays==1
                ...
                newrowtomake1=1;newrowtomake2=1;curhw=1;
                for j=1:size(fullhwdaystoplot,1)
                    if moistness==1
                        ...
                        if scorescomptotalhw(curhw+15,2)>=medianWBT %very moist heat wave (the first 15 in scorescomptotalhw are pre-1979)
    ...
    if moistness==1 %here firstday is preset to 99
        ...
        for relmoist=1:2 %very moist & not-so-moist heat waves
            for variab=viwf:viwl
                ...
                %Assign hourly sums directly to holding array
                %Dimensions of avgsmoistness are variable|pressure level|stdhour
                if varsdes(variab)==2
                    for hr=1:size(hourstodo,1) %not interested in all the hours
        ...
        save('/Users/colin/Documents/General_Academics/Research/Exploratory_Plots/Saved_Variables_etc/tempvarholder2','avgsmoistness');
    end
end
...
if savetotalsintoarrays==1
    ...
    for variab=viwf:viwl
        if strcmp(varlist(varsdes(variab)),'air')
            eval(['arr1f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(1)) 'cl0=total;']); %i.e. all the days (no clustering at all)
            %Data for all first days OR last days in each bottom-up cluster
            if computeusebottomupclusters==1
                for cl=1:k;eval(['arr1f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(1)) 'cl' num2str(cl) '=totalcl' num2str(cl) ';']);end
                cl=99;eval(['arr1f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(1)) 'cl' num2str(cl) '=totalcl' num2str(cl) ';']);
            elseif allhwdays==1
                for cl=clmin:clmax
                    eval(['arr1f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(1)) 'cl' num2str(cl) '=avgsallhwdays{1,presleveliw(1),10};']);
                end
            %Data for the 3 temporal-evolution 'clusters'
            elseif tempevol==1
                for cl=clmin:clmax
                    firstday=cl-fdo;eval(['arr1f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(1)) 'cl' num2str(cl) '='...
                        'avgstempevol{1,presleveliw(1),' num2str(cl) ',10};']);
                end
            %Data for the 3 seasonality 'clusters'
            elseif seasonality==1
                for cl=clmin:clmax
                    season=cl-fdo;eval(['arr1f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(1)) 'cl' num2str(cl) '='...
                        'avgsseasonality{1,presleveliw(1),' num2str(season) ',10};']);
                end
                %cl=299;firstday=99;eval(['arr1f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(1)) 'cl' num2str(cl) '='...
                %'avgsseasonality{1,presleveliw(1),' num2str(cl) ',10};']);
            %Data for the 4 diurnality 'clusters'
            elseif diurnality==1
                for cl=clmin:clmax
                    hour=cl-fdo;actualhour=hourstodo(hour);
                    eval(['arr1f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(1)) 'cl' num2str(cl) '='...
            ...
            if computeusebottomupclusters==1
                if max(max(totalfirstcl1))~=0 && max(max(totallastcl1))~=0
                    for cl=1:k
                        eval(['arr1f99rb' num2str(vartorankby) 'pl' preslevels(presleveliw(1)) 'cl' num2str(cl) '=(totalfirstcl'...
        ...
        elseif strcmp(varlist(varsdes(variab)),'shum') %uses shum but output is WBT
            eval(['arr2f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(2)) 'cl0=totalwbt;']);
            if computeusebottomupclusters==1
                for cl=1:k;eval(['arr2f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(2)) 'cl' num2str(cl) '=totalwbtcl' num2str(cl) ';']);end
                cl=99;eval(['arr2f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(2)) 'cl' num2str(cl) '=totalwbtcl' num2str(cl) ';']);
            elseif allhwdays==1
                for cl=clmin:clmax
                    eval(['arr2f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(2)) 'cl' num2str(cl) '=avgsallhwdays{2,presleveliw(2),10};']);
                end
            elseif tempevol==1
                for cl=clmin:clmax
                    firstday=cl-fdo;eval(['arr2f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(2)) 'cl' num2str(cl) '='...
                        'avgstempevol{2,presleveliw(2),' num2str(cl) ',10};']);
                end
            elseif seasonality==1
                for cl=clmin:clmax
                    season=cl-fdo;eval(['arr2f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(2)) 'cl' num2str(cl) '='...
                        'avgsseasonality{2,presleveliw(2),' num2str(season) ',10};']);
                end
                %cl=299;firstday=99;eval(['arr2f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(2)) 'cl' num2str(cl) '='...
                %'avgsseasonality{2,presleveliw(2),' num2str(cl) ',10};']);
            elseif diurnality==1
                for cl=clmin:clmax
                    hour=cl-fdo;actualhour=hourstodo(hour);
                    eval(['arr2f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(2)) 'cl' num2str(cl) '='...
            ...
            if computeusebottomupclusters==1
                if max(max(totalwbtfirstcl1))~=0 && max(max(totalwbtlastcl1))~=0
                    for cl=1:k
                        eval(['arr2f99rb' num2str(vartorankby) 'pl' preslevels(presleveliw(2)) 'cl' num2str(cl) '=(totalwbtfirstcl'...
        ...
        elseif strcmp(varlist(varsdes(variab)),'hgt')
            eval(['arr3f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(3)) 'cl0=total;']);
            if computeusebottomupclusters==1
                for cl=1:k;eval(['arr3f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(3)) 'cl' num2str(cl) '=totalcl' num2str(cl) ';']);end
                cl=99;eval(['arr3f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(3)) 'cl' num2str(cl) '=totalcl' num2str(cl) ';']);
            elseif allhwdays==1
                for cl=clmin:clmax
                    eval(['arr3f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(3)) 'cl' num2str(cl) '=avgsallhwdays{3,presleveliw(3),10};']);
                end
            elseif tempevol==1
                for cl=clmin:clmax
                    firstday=cl-fdo;eval(['arr3f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(3)) 'cl' num2str(cl) '='...
                        'avgstempevol{3,presleveliw(3),' num2str(cl) ',10};']);
                end
            elseif seasonality==1 %all with firstday=99
                for cl=clmin:clmax
                    season=cl-fdo;eval(['arr3f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(3)) 'cl' num2str(cl) '='...
                        'avgsseasonality{3,presleveliw(3),' num2str(season) ',10};']);
                end
            elseif diurnality==1
                for cl=clmin:clmax
                    hour=cl-fdo;actualhour=hourstodo(hour);
                    eval(['arr3f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(3)) 'cl' num2str(cl) '='...
            ...
            if computeusebottomupclusters==1
                if max(max(totalfirstcl1))~=0 && max(max(totallastcl1))~=0
                    for cl=1:k
                        eval(['arr3f99rb' num2str(vartorankby) 'pl' preslevels(presleveliw(3)) 'cl' num2str(cl) '=(totalfirstcl'...
                            num2str(cl) '*hotdaysfirstccl' num2str(cl) '+totallastcl' num2str(cl) '*hotdayslastccl'...
                            num2str(cl) ')/(hotdaysfirstccl' num2str(cl) '+hotdayslastccl' num2str(cl) ');']);
                    end
                end
        ...
        elseif strcmp(varlist(varsdes(variab)),'uwnd')
            eval(['arr4f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(4)) 'cl0=total;']);
            if computeusebottomupclusters==1
                for cl=1:k;eval(['arr4f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(4)) 'cl' num2str(cl) '=totalcl' num2str(cl) ';']);end
                cl=99;eval(['arr4f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(4)) 'cl' num2str(cl) '=totalcl' num2str(cl) ';']);
            elseif allhwdays==1
                for cl=clmin:clmax
                    eval(['arr4f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(4)) 'cl' num2str(cl) '=avgsallhwdays{4,presleveliw(4),10};']);
                end
            elseif tempevol==1
                for cl=clmin:clmax
                    firstday=cl-fdo;eval(['arr4f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(4)) 'cl' num2str(cl) '='...
                        'avgstempevol{4,presleveliw(4),' num2str(cl) ',10};']);
                end
            elseif seasonality==1
                for cl=clmin:clmax
                    season=cl-fdo;eval(['arr4f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(4)) 'cl' num2str(cl) '='...
                        'avgsseasonality{4,presleveliw(4),' num2str(season) ',10};']);
                end
                %cl=299;firstday=99;eval(['arr4f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(4)) 'cl' num2str(cl) '='...
                %'avgsseasonality{4,presleveliw(4),' num2str(cl) ',10};']);
            elseif diurnality==1
                for cl=clmin:clmax
                    hour=cl-fdo;actualhour=hourstodo(hour);
                    eval(['arr4f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(4)) 'cl' num2str(cl) '='...
            ...
            if computeusebottomupclusters==1
                if max(max(totalfirstcl1))~=0 && max(max(totallastcl1))~=0
                    for cl=1:k
                        eval(['arr4f99rb' num2str(vartorankby) 'pl' preslevels(presleveliw(4)) 'cl' num2str(cl) '=(totalfirstcl'...
                            num2str(cl) '*hotdaysfirstccl' num2str(cl) '+totallastcl' num2str(cl) '*hotdayslastccl'...
                            num2str(cl) ')/(hotdaysfirstccl' num2str(cl) '+hotdayslastccl' num2str(cl) ');']);
                    end
                end
        ...
        elseif strcmp(varlist(varsdes(variab)),'vwnd')
            eval(['arr5f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(5)) 'cl0=total;']);
            if computeusebottomupclusters==1
                for cl=1:k;eval(['arr5f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(5)) 'cl' num2str(cl) '=totalcl' num2str(cl) ';']);end
                cl=99;eval(['arr5f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(5)) 'cl' num2str(cl) '=totalcl' num2str(cl) ';']);
            elseif allhwdays==1
                for cl=clmin:clmax
                    eval(['arr5f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(5)) 'cl' num2str(cl) '=avgsallhwdays{5,presleveliw(5),10};']);
                end
            elseif tempevol==1
                for cl=clmin:clmax
                    firstday=cl-fdo;eval(['arr5f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(5)) 'cl' num2str(cl) '='...
                        'avgstempevol{5,presleveliw(5),' num2str(cl) ',10};']);
                end
            elseif seasonality==1
                for cl=clmin:clmax
                    season=cl-fdo;eval(['arr5f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(5)) 'cl' num2str(cl) '='...
                        'avgsseasonality{5,presleveliw(5),' num2str(season) ',10};']);
                end
                %cl=299;firstday=99;eval(['arr5f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(5)) 'cl' num2str(cl) '='...
                %'avgsseasonality{5,presleveliw(5),' num2str(cl) ',10};']);
            elseif diurnality==1
                for cl=clmin:clmax
                    hour=cl-fdo;actualhour=hourstodo(hour);
                    eval(['arr5f' num2str(firstday) 'rb' num2str(vartorankby) 'pl' preslevels(presleveliw(5)) 'cl' num2str(cl) '='...
            ...
            if computeusebottomupclusters==1
                if max(max(totalfirstcl1))~=0 && max(max(totallastcl1))~=0
                    for cl=1:k
                        eval(['arr5f99rb' num2str(vartorankby) 'pl' preslevels(presleveliw(5)) 'cl' num2str(cl) '=(totalfirstcl'...
                            num2str(cl) '*hotdaysfirstccl' num2str(cl) '+totallastcl' num2str(cl) '*hotdayslastccl'...
                            num2str(cl) ')/(hotdaysfirstccl' num2str(cl) '+hotdayslastccl' num2str(cl) ');']);
                    end
                end
...
if plotheatwavecomposites==1
    ...
    if overlay==1 && showoverlaycompositeplots==1 %Make single or double overlay
        ...
        elseif overlaynum==1 || overlaynum==2 || overlaynum==3 %scalars
            ...
            overlaydata2={lats;lons;(arr4f99rb1cl513-narravgjjauwnd)-(arr4f99rb1cl505-narravgjjauwnd);(arr5f99rb1cl513-narravgjjavwnd)-(arr5f99rb1cl505-narravgjjavwnd)};
                ...
                for dayinmonth=1:curmonlen
                    ...
                    if strcmp(varlist{varsdes(arealvariab)},'shum')
                        for hr=1:8
                            ...
                            wsArr=0.622*esArr/1000;
                            rhArr=100*mrArr./wsArr;
                            eval(['arrDOI' hours{hr} '=arrDOIprevthishr.*atan(0.151977.*(rhArr+8.313659).^0.5)+'...
                                'atan(arrDOIprevthishr+rhArr)-atan(rhArr-1.676331)+'...
                                '0.00391838.*(rhArr.^1.5).*atan(0.0231.*rhArr)-4.686035;']);
                            if dayinmonth~=eval(['m' num2str(month+1) 's-m' num2str(month) 's']) %i.e. ~= last day of month
                                arrDOIprevthishr=eval(['prevArr{3}(:,:,presleveliw(varsdes(arealvariab)),(dayinmonth+1)*8+ ' num2str(hr-8) ')-273.15;']); %T
                                eval(['arrDOI' hours{hr} 'nextday=arrDOIprevthishr.*atan(0.151977.*(rhArr+8.313659).^0.5)+'...
                                'atan(arrDOIprevthishr+rhArr)-atan(rhArr-1.676331)+'...
                                '0.00391838.*(rhArr.^1.5).*atan(0.0231.*rhArr)-4.686035;']);
                            else
