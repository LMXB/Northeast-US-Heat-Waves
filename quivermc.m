...
% v is the meridional component where northward is positive.  Matlab gets this wrong. 
% 
...
tmp=strcmpi(varargin,'maparea');
map_area=varargin{2};
%disp('line 241');disp(map_area);
sq_map_area=sqrt(map_area);%disp(sq_map_area);

if map_area<=12 %i.e. mapregion is nyc-area
    skipstep=12;
end

%disp(skipstep); %the default calculated just above
tmp=strcmpi(varargin,'skipstep');
skipstepnew=varargin{4};%disp('line 251');disp(skipstep);disp(skipstepnew);
lat = lat(1:skipstepnew:end,1:skipstepnew:end); 
lon = lon(1:skipstepnew:end,1:skipstepnew:end);
u = u(1:skipstepnew:end,1:skipstepnew:end); 
v = v(1:skipstepnew:end,1:skipstepnew:end); 
...
%Scale the vectors according to
%the mean distance between gridpoints. This is a good measure, as it remains 
...
u=u(~isnan(x))';v=v(~isnan(x))';
y=y(~isnan(x))';x=x(~isnan(x))';

% Set arrow size (1=full length of vector)
arrow=0.40;

% set scale value based on refval, scale length, and map area
%larger scale --> bigger arrows all else equal
%24 degrees squared is the empirically derived 'ideal area' for which the reference vector
%looks just right with no tweaking whatsoever
roundp=floor(log10(refval));
refval=floor(refval/(10^roundp))*(10^roundp);
scale=scalelength/refval;
scale=scale/24*sq_map_area*(skipstep/skipstepnew);
%scale=scale/24*sq_map_area; 
    %scale is further adjusted for any arrow-density adjustments that were made by assigning skipstepnew
...
%Not using this loop -- see the loop just below for my own version
if refvec
 ...
 xdist=scalelength*skipstep/skipstepnew;
 %xdist=scalelength;
 x=[0.9-xdist 0.9]; %start and end of arrow respectively
 y=[0.43 0.43];
